...
>
  ...
  const Commissions = () => (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold">Mes Commissions</h2>
        <div className="flex gap-2">
          <button
            disabled={pendingCommission <= 0}
            onClick={() => handleRequest("payout")}
          ...
          <button
            disabled={pendingCommission <= 0}
            onClick={() => setOpenBoutique(true)}
            className={`px-3 py-2 rounded-lg text-white ${
              pendingCommission > 0
                ? "bg-green-600 hover:bg-green-700"
                : "bg-gray-400 cursor-not-allowed"
            }`}
            title="Utiliser la commission en boutique"
          >
      ...
      {/* Full commissions list (same module as user dashboard) */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow p-4">
        <UserCommissions userId={profile?.id} />
      </div>

      {/* Latest commission requests list (same module stack as user dashboard) */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow p-4">
        <h3 className="font-semibold mb-3">Dernières demandes</h3>
        <UserCommissionsRequests userId={profile?.id} />
      </div>

      {/* Optional boutique pane */}
      {openBoutique && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow p-4">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold flex items-center gap-2">
              <FaShoppingBag /> Utiliser mes commissions en boutique
            </h3>
            <button
              onClick={() => setOpenBoutique(false)}
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className="w-64 bg-gray-900 shadow-lg flex flex-col">
        <div className="p-4 border-b border-gray-700 flex flex-col items-center">
          <img src="/logo/aquador.png" alt="Logo" className="h-10 w-10 mb-1" />
