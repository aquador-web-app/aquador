...
async function fetchSessions() {
  ...
  const fetchStats = async () => {
    ...
    const rows = invs || [];
    const unpaid = rows.filter(r => (Number(r.total || 0) - Number(r.paid_total || 0)) > 0);
    const unpaidCount = unpaid.length;
    const unpaidTotal = unpaid.reduce(
      (s, r) => s + Math.max(0, Number(r.total || 0) - Number(r.paid_total || 0)),
      0
    );
  ...
  return (
    <div>
      <div className="flex items-center space-x-3 mb-6">
        <h2 className="text-2xl font-bold text-aquaBlue">A'QUA D'OR - Admin Dashboard</h2>
      </div>
...
>
  ...
  >
    ...
    <p className="text-sm text-gray-600">Total: {formatCurrencyUSD(unpaidInvoices.total)}</p>
  </motion.div>
  ...
  <motion.div
    ...
    whileHover={{ scale: 1.03, y: -3 }}
  >
    <div className="absolute top-0 left-0 h-1 w-full bg-gradient-to-r from-green-500 to-emerald-400 rounded-t-2xl"></div>
    <p className="text-gray-500 font-medium">Présences du mois</p>
    <h3 className="text-3xl font-bold text-green-600">{attendance.percent}%</h3>
    <p className="text-sm text-gray-600">Total: {attendance.total}</p>
  </motion.div>
...
<div>
  <button
    onClick={() =>
      setActiveTab(
        activeTab.startsWith("bulletins") || activeTab.startsWith("fiches")
          ? ""
          : "bulletins-main"
      )
    }
  ...
  >
    {/* left side: fixed icon width + no wrap + truncate (matches your other menus) */}
    ...
    {/* right caret aligned like others */}
  ...
  className={`flex items-center gap-2 text-left px-2 py-1 rounded ${
    activeTab === "bulletinsetFiches" ? "bg-aquaBlue text-white" : "text-gray-100 hover:bg-orange-700"
    }`}
      >
  ...
  className={`flex items-center gap-2 text-left px-2 py-1 rounded ${
    activeTab === "bulletins-form" ? "bg-aquaBlue text-white" : "text-gray-100 hover:bg-orange-700"
    }`}
      >
...
<button
        ...
        className={`flex items-center gap-2 text-left px-2 py-1 rounded ${
          activeTab === "fiches-techniques" ? "bg-aquaBlue text-white" : "text-gray-100 hover:bg-orange-700"
        }`}
      >
      ...
      <button
        ...
        className={`flex items-center gap-2 text-left px-2 py-1 rounded ${
          activeTab === "bulletins-template" ? "bg-aquaBlue text-white" : "text-gray-100 hover:bg-orange-700"
        }`}
      >
      ...
      <button
        ...
        className={`flex items-center gap-2 text-left px-2 py-1 rounded ${
          activeTab === "fiches-template" ? "bg-aquaBlue text-white" : "text-gray-100 hover:bg-orange-700"
        }`}
      >
